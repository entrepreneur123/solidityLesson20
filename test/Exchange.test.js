...
contract("Exchange", ([deployer, feeAccount, user1, user2]) => {
  ...
  describe('withdrawing Ether', async()=> {
      ...
      beforeEach(async()=> {
          amount = ether(1)
          //deposit Ether First
          await exchange.depositeEther({from:user1, value: amount})
      })
  ...
  describe('checking balances', async()=> {
      beforeEach(async()=> {
          exchange.depositeEther({ from: user1, value: ether(1)})
...
describe('making orders',async()=> {
    let result
    beforeEach(async ()=> {
        result = await exchange.makeOrder(token.address(1), ETHER_ADDRESS, ether(1), {from: user1})
    })
    ...
    it('tracks the newly created order', async()=> {
        ...
        order.amountGet.toString().should.equal(token(1).toString(),'amountGet is correct')
    ...
    it('emits on "Order" event', async ()=> {
        ...
        event.amountGet.toString().should.equal(token(1).toString(), 'amountGet is correct')
...
describe('filling orders', async()=> {
    ...
    describe('success',async()=> {
        ...
        it('executes the trade & charges fees', async()=> {
            ...
            balance.toString().should.equal(token(1).toString(), 'user1 received tokens')
            balance = await exchange.balanceOf(ETHER_ADDRESS, user2 )
            balance = toString().should.equal(ether(1).toString(), 'user2 received Ether')
            balance = await exchange.balanceOf(ETHER_ADDRESS,user1)
            balance.toString().should.equal('0', 'user2 Ether deduced')
            balance = await exchange.balanceOf(token.address, user2)
            balance.toString().should.equal(tokens(0.9).toString(), 'user2 tokens deduced with fee applied')
            const feeAccount = await exchange.feeAccount()
            balance = await exchange.balanceOf(token.address, feeAccount)
            balance.toString().should.equal(token(0.1).toString(), 'feeAccount received fee')
        })
        ...
        it('emits a "trade" event', async()=> {
            ...
            event.tokenGiven.should.equal(ETHER_ADDRESS, 'tokenGive is correct')
            event.amountGive.toString().should.equal(ether(1).toString(),'amountGive is correct')
    ...
    describe('failure', async()=> {
        ...
        it('rejects unauthorized cancelation',async()=> {
            //try to cancel the order from another 
